...
# The bootloader's proof of possession of its own signing key, over a phone
# nonce (GET /v0.4/attest). What the phone trusts INSTEAD of a TLS leaf pin.
ATTEST_ACTION = "bootloader:attest"
ATTEST_WINDOW_SECONDS = 120
# The bootloader's signature over a /v0.4/pending envelope (server-signed cards):
# the phone renders no card it cannot verify. Closes the channel-binding gap the
# 09-16 leaf-pin retirement opened (a CA-issued interposer could relay attest and
# forge a card; now it cannot make one render).
PENDING_ACTION = "bootloader:pending"
PENDING_WINDOW_SECONDS = 300
# Two clocks never agree to the second. Everything the bootloader signs is
# valid from CLOCK_SKEW_SECONDS in the past (nbf = now - skew; exp unchanged):
# the first server-signed card in the field (2026-09-16, Pixel one second
# behind darwin) was refused as "nbf is in the future".
