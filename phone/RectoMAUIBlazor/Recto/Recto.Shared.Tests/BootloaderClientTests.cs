...
public class BootloaderClientTests
{
    ...
    // THE LABEL IS NOT THE SENTENCE (2026-09-21): a BootloaderError answers {"error": "bootloader_error",
    // "detail": "<why>"}; the phone showed the label alone on an approve of an expired card. The detail rides.
    [Fact]
    public async Task A_bootloader_error_surfaces_its_detail_not_just_its_label()
    {
        var handler = new RecordingHandler(_ => Json(HttpStatusCode.BadRequest,
            """{"error": "bootloader_error", "detail": "request req-1 expired at 1790003906"}"""));
        var sut = MakeClient(handler);

        var result = await sut.RespondAsync("http://h", "req-1",
            new RespondRequest(PhoneId: "phone-id-1", Decision: "approved", SignatureB64u: "sig"), CancellationToken.None);

        Assert.True(result.IsFailure);
        Assert.Contains("bootloader_error", result.Error.Message);
        Assert.Contains("expired at 1790003906", result.Error.Message);
    }

    [Fact]
    public async Task A_bootloader_error_without_detail_still_shows_its_label()
    {
        var handler = new RecordingHandler(_ => Json(HttpStatusCode.BadRequest, """{"error": "unknown_endpoint"}"""));
        var sut = MakeClient(handler);
        ...
        Assert.True(result.IsFailure);
        Assert.Contains("unknown_endpoint", result.Error.Message);
    }
