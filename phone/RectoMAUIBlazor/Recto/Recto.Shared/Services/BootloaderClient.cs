...
public sealed class BootloaderClient : IBootloaderClient
{
    ...
    public Task<Result<AttestResponse>> AttestAsync(
        string bootloaderUrl, string nonceB64u, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(bootloaderUrl))
        {
            return Task.FromResult(Result.Failure<AttestResponse>(
                Error.Validation([new ValidationErrors("bootloaderUrl", "Bootloader URL is required.")])));
        }

        if (string.IsNullOrWhiteSpace(nonceB64u))
        {
            return Task.FromResult(Result.Failure<AttestResponse>(
                Error.Validation([new ValidationErrors("nonce", "Nonce is required.")])));
        }

        var url = $"{bootloaderUrl.TrimEnd('/')}/v0.4/attest?nonce={Uri.EscapeDataString(nonceB64u)}";
        return SendAsync<AttestResponse>(HttpMethod.Get, url, body: null, ct);
    }
    ...
    /// <summary>
    /// /v0.4/pending is the one response whose RAW bytes matter: the bootloader's
    ...
    private async Task<Result<PendingRequestsResponse>> GetPendingWithRawAsync(string url, string phoneId, CancellationToken ct)
    {
        ...
        try
        {
            var parsed = JsonSerializer.Deserialize<PendingRequestsResponse>(body.Value, _readOptions);
            if (parsed is null)
            {
                return Result.Failure<PendingRequestsResponse>(Error.Failure("Bootloader returned an empty response body."));
            }
            string? raw = null;
            ...
            return Result.Success(parsed with { RequestsRawJson = raw });
        }
        catch (JsonException ex)
        {
            return Result.Failure<PendingRequestsResponse>(Error.Failure($"Bootloader returned malformed JSON: {ex.Message}"));
        }
    }
        ...
        PollSignatureHeaders? readSignature = null) where T : class
    {
        ...
        try
        {
            var parsed = JsonSerializer.Deserialize<T>(text.Value, _readOptions);
            if (parsed is null)
            {
        ...
        {
            _log.LogWarning(ex, "Bootloader {Method} {Url} malformed JSON", method, url);
            return Result.Failure<T>(Error.Failure($"Bootloader returned malformed JSON: {ex.Message}"));
        }
    }
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                if (!string.IsNullOrEmpty(friendlyMessage))
                {
                    ...
                    return Result.Failure<string>(Error.Failure($"{statusHint}: {friendlyMessage}"));
                }
            ...
            {
                return Result.Failure<string>(Error.Failure("Bootloader returned an empty response body."));
            }
            return Result.Success(text);
        }
        catch (OperationCanceledException) when (ct.IsCancellationRequested)
        {
            return Result.Failure<string>(Error.Failure("Operation was canceled."));
        }
        catch (TaskCanceledException)
        {
            return Result.Failure<string>(Error.Failure(TimedOutMessage));
        }
        catch (HttpRequestException ex)
        {
            _log.LogWarning(ex, "Bootloader {Method} {Url} network error", method, url);
            return Result.Failure<string>(Error.Failure($"{UnreachablePrefix}: {ex.Message}"));
        }
        catch (Exception ex)
        {
    ...
    private static string? TryExtractServerErrorMessage(string body)
    {
        ...
        try
        {
            ...
            // THE LABEL IS NOT THE SENTENCE (2026-09-21): the bootloader answers {"error": "bootloader_error",
            // "detail": "<why>"} for every BootloaderError, and the phone showed the label - "HTTP 400:
            // bootloader_error" on an approve of an expired card, read for one tick and gone. The detail is the
            // bootloader's own words about THIS refusal; it rides along, label first.
            if (doc.RootElement.TryGetProperty("detail", out var detailProp)
                && detailProp.ValueKind == JsonValueKind.String
                && detailProp.GetString() is { Length: > 0 } detail)
            {
                return $"{message}: {(detail.Length > 300 ? detail[..300] : detail)}";
            }
            return message;
        }
        catch (JsonException)
