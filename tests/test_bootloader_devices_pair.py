...
class TestDevicesPairSignedResult:
    ...
    def test_signing_bootloader_forwards_a_result_that_verifies_and_pins_the_act(self, bootloader, fake_consumer, phone):
        ...
        assert claims.exp - claims.nbf == 360   # 300 s window + 60 s clock skew behind
...
class TestBootloaderAttest:
    """GET /v0.4/attest: the server proves it holds the key the phone pinned.

    Born 2026-09-16 from the Cloudflare edge-certificate reissue that bricked
    every paired phone below HTTP: a TLS leaf pin is the vendor's lease, the
    signing key is our identity. The phone drops the leaf pin for THIS.
    """
    ...
    def test_signature_over_the_nonce_verifies_against_the_published_key(self, bootloader):
        import hashlib
        from recto.capability.jwt import verify_jws
        from recto.capability.pair_record import ATTEST_ACTION, ATTEST_WINDOW_SECONDS
        from recto.capability.signing import public_key_hex

        ctx = bootloader(consumer_tokens={"https://x.com": "tok"}, signing_key=_BOOTLOADER_KEY)
        nonce = b"\x5a" * 32
        nonce_b64u = _b64u(nonce)
        status, body = self._get(f"{ctx['base_url']}/v0.4/attest?nonce={nonce_b64u}")
        assert status == HTTPStatus.OK
        assert body["bootloader_id"] == _BOOTLOADER_ID
        assert body["nonce"] == nonce_b64u
        assert body["devices_pair_pubkey"] == public_key_hex(_BOOTLOADER_KEY)
        claims = verify_jws(body["attest_jws"], expected_pubkey=bytes.fromhex(public_key_hex(_BOOTLOADER_KEY)),
                            expected_aud="recto-phone")
        assert claims.iss == f"bootloader:{_BOOTLOADER_ID}"
        assert claims.cap.scope.payload_sha256 == hashlib.sha256(nonce).hexdigest()
        assert claims.cap.allow_actions == [ATTEST_ACTION]
        assert claims.exp - claims.nbf == ATTEST_WINDOW_SECONDS + 60   # + CLOCK_SKEW_SECONDS behind
        assert claims.nbf <= claims.iat - 60

    def test_another_key_does_not_verify(self, bootloader):
        from recto.capability.jwt import verify_jws
        from recto.capability.signing import public_key_hex

        ctx = bootloader(consumer_tokens={"https://x.com": "tok"}, signing_key=_BOOTLOADER_KEY)
        nonce = _b64u(b"\x01" * 16)
        _, body = self._get(f"{ctx['base_url']}/v0.4/attest?nonce={nonce}")
        other = bytes.fromhex("8" * 64)
        with pytest.raises(Exception):
            verify_jws(body["attest_jws"], expected_pubkey=bytes.fromhex(public_key_hex(other)))

    @pytest.mark.parametrize("nonce", ["", "AAAA", "!!!!", _b64u(b"\x02" * 15), _b64u(b"\x02" * 65)])
    def test_a_bad_nonce_is_refused_by_name(self, bootloader, nonce):
        ctx = bootloader(consumer_tokens={"https://x.com": "tok"}, signing_key=_BOOTLOADER_KEY)
        status, body = self._get(f"{ctx['base_url']}/v0.4/attest?nonce={nonce}")
        assert status == HTTPStatus.BAD_REQUEST
        assert body["error"] == "bad_nonce"

    def test_an_unsigned_bootloader_cannot_attest(self, bootloader):
        ctx = bootloader(consumer_tokens={"https://x.com": "tok"})
        nonce = _b64u(b"\x03" * 16)
        status, body = self._get(f"{ctx['base_url']}/v0.4/attest?nonce={nonce}")
        assert status == HTTPStatus.SERVICE_UNAVAILABLE
        assert body["error"] == "attest_unavailable"

    def test_register_carries_the_pubkey_the_phone_pins(self, bootloader):
        """The phone captures the key in the pairing window it already trusts."""
        from recto.capability.signing import public_key_hex

        ctx = bootloader(consumer_tokens={"https://x.com": "tok"}, signing_key=_BOOTLOADER_KEY)
        # _handle_register's response is built from cfg; assert on the shape
        # via the health twin here and the register path in test_bootloader.py.
        _, body = self._get(f"{ctx['base_url']}/v0.4/health")
        assert body["devices_pair_pubkey"] == public_key_hex(_BOOTLOADER_KEY)
